...
outputFolder = "output3\" + imgFolder+"_output";
binaryFolder = "binary3\" + imgFolder+"_binary";
...
for l = 1 : numImg
    ...
    meanValue = graythresh(grayImage);
    % thresholding
    for i=1:x
        for j=1:y
            if grayImage(i, j) <= meanValue * 256
                grayImage(i, j) = 1.0;
            else 
                grayImage(i, j) = 0.0;
            end
    ...
    for i=1:a
        for j=1:b
            if(binaryImage(i, j)<= meanValue * 256)
                %grayImage(i, j) = 256;
                %binaryImage(i,j) = 256;
                binaryImage(i,j) = 256;
            else
